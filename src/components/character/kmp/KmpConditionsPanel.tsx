...
// Reuse the constant list if possible, otherwise duplicating for now to be safe
...
export function KmpConditionsPanel() {
    const { viewModel, state } = useKmpCharacter();
    const [isOpen, setIsOpen] = useState(false);
    const [query, setQuery] = useState("");

    const activeConditions = state?.resolvedConditions || [];
    // Map existing active IDs
    const activeIds = activeConditions.map((ac: any) => ac.condition.id);
                                            ...
                                            {filteredConditions.map((c) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        onClick={() => viewModel?.toggleCondition(c.id)}
                                                    >
                ...
                <CardContent className="py-2">
                    {activeConditions.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-1">
                            {activeConditions.map((entry: any) => {
                                const c = entry.condition;
                                const conditionData = CONDITIONS_LIST.find(cond => cond.id === c.id);
                                // Use entry.id (Database ID) for key if valid (>0), otherwise fallback to conditionId + random (less safe but works for optimistic)
                                // Actually, entry.id might be 0 for optimistic updates. 
                                // Best to use `${c.id}-${entry.id}` to ensure uniqueness if multiple same conditions exist (rare but possible)
                                const uniqueKey = entry.id > 0 ? entry.id : `${c.id}-optimistic`;
                                return (
